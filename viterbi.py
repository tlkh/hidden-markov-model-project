...
class HMM(object):
    ...
    def _viterbi(self, seq_x_words):
        ...
        def pi(j, v):
            ...
            word = seq_x[j-1]
            if j == n:
                pi_list = [self.SCALE * pi(j-1, u) * self.a(u, self.get_stop_token()) for u in range(2, len_y_vocab)]
                max_pi = max(pi_list)
            elif j > 1:
                pi_list = [self.SCALE * pi(j-1, u) * self.b(u, word) * self.a(u, v) for u in range(2, len_y_vocab)]
                max_pi = max(pi_list)
            elif j == 1:
            ...
            return max_pi    
        def backtrack(j, v):
            probs = [self.SCALE * pi(j, u) * self.a(u, v) for u in range(2, len_y_vocab)]
